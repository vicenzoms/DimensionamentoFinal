...
#  OPTIMIZER MA
    ...
    st.write("Insira os parâmetros tradicionais e os referentes à impressão 3D para calcular o mix ideal.")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.markdown("**Parâmetros Tradicionais**")
        L = st.number_input("Lambda Tradicional (taxa de falha):", min_value=0.0000, value=0.05, step=0.01, format="%.6f", key="L_trad")
        N = st.number_input("Número de máquinas ativas (n):", min_value=1, value=10, step=1, key="N_trad")
        T = st.number_input("Tempo de reposição (t):", min_value=1.0, value=10.0, step=1.0, key="T_trad")
        R_PCT = st.number_input("Risco Alvo (%):", min_value=0.01, max_value=99.99, value=5.00, step=1.0, format="%.2f", key="R_trad")
        custo_unitario = st.number_input("Custo Unitário Tradicional (R$):", min_value=0.00, value=150.00, step=10.00, format="%.2f", key="C_trad")

    with col2:
        st.markdown("**Parâmetros Impressão 3D**")
        L_3d = st.number_input("Lambda 3D (taxa de falha):", min_value=0.0000, value=0.08, step=0.01, format="%.6f", key="L_3d")
        T_3d = st.number_input("Tempo de impressão por peça:", min_value=0.1, value=1.0, step=0.5, key="T_3d")
        custo_3d = st.number_input("Custo Unitário de Impressão (R$):", min_value=0.00, value=50.00, step=10.00, format="%.2f", key="C_3d")

    botao_ma = st.button("Calcular Otimização Aditiva")
    
    if botao_ma:
        risco = R_PCT / 100.0
        
        # 1. Otimização Padrão para achar o X Tradicional
        df_p, x_trad, m_trad = calcular_poisson(L, N, T, risco)
        custo_total_tradicional = x_trad * custo_unitario
        
        # 2. Base da otimização aditiva (x / 2)
        x_base = int(x_trad // 2)
        
        # 3. Loop para descobrir quantas peças 3D são necessárias
        x_3d = 0
        valido = True
        motivo_invalido = ""
        ...
        while True:
            ...
            # Evita divisão por zero caso a base seja 0
            if x_tot == 0:
                L_avg = L
            else:
                L_avg = ((x_base * L) + (x_3d * L_3d)) / x_tot
            
            # O sistema continua sendo avaliado dentro da janela do tempo de reposição T
            m_sys = L_avg * N * T 
            
            # Calcula o risco com o total de peças
            from scipy.stats import poisson
            prob_acumulada = poisson.cdf(x_tot, m_sys)
            risco_atual = 1.0 - prob_acumulada
            
            if risco_atual <= risco:
                break
                
            x_3d += 1
            
            # Restrição 1: tempo total de impressão não deve superar tempo de reposição T
            if (x_3d * T_3d) > T:
                valido = False
                motivo_invalido = f"Restrição de Tempo: O tempo total de impressão ({x_3d * T_3d}) ultrapassou o tempo de reposição tradicional ({T})."
                break
                
        if valido:
            # Restrição 2: Custo total MA <= Custo Tradicional
            custo_total_ma = (x_base * custo_unitario) + (x_3d * custo_3d)
            
            if custo_total_ma > custo_total_tradicional:
                valido = False
                motivo_invalido = f"Restrição Financeira: O custo da Manutenção Aditiva (R$ {custo_total_ma:.2f}) ficou maior que o custo da Tradicional (R$ {custo_total_tradicional:.2f})."
        
        st.divider()
        
        # Tratamento dos Outputs
        if not valido:
            st.error("A Otimização de Manutenção Aditiva **não é a melhor forma de otimização** para este cenário.")
            st.warning(motivo_invalido)
            st.info(f"Recomendação Tradicional: **{x_trad} peças** com custo total de **R$ {custo_total_tradicional:.2f}**.")
        else:
            custo_total_ma = (x_base * custo_unitario) + (x_3d * custo_3d)
            
            st.success("Otimização Aditiva viável encontrada com sucesso!")
            
            # Métricas de Peças
            col_res1, col_res2, col_res3 = st.columns(3)
            col_res1.metric("Peças Tradicionais (Base)", f"{x_base}")
            col_res2.metric("Peças Impressas (3D)", f"{x_3d}")
            col_res3.metric("Total de Peças", f"{x_base + x_3d}")
            
            # Métricas de Custo
            col_c1, col_c2 = st.columns(2)
            col_c1.metric("Custo Total (Híbrido)", f"R$ {custo_total_ma:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
            col_c2.metric("Economia vs Tradicional", f"R$ {(custo_total_tradicional - custo_total_ma):,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
            
            # Informações adicionais do sistema
            st.markdown(f"**Detalhes Operacionais:**")
            st.markdown(f"- **Risco final do sistema:** {risco_atual:.4%} *(Alvo: {R_PCT}%)*")
            st.markdown(f"- **Tempo de impressão comprometido:** {x_3d * T_3d} tempo(s) *(Máximo permitido: {T})*")
            st.markdown(f"- **Valor Esperado de Falhas Ajustado (m):** {m_sys:.2f}")
